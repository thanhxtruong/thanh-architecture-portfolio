...
An application asks an external system to create a contract. It learns the result through two independent channels: the request's response and an asynchronous notification. When the response times out, the notification can arrive before the application has recorded the external identifier. The two paths can then associate the same operation with different local records. A subsequent persistence failure exposes another problem: the workflow knows how to repeat the request, but lacks a reliable way to resume after external success.
 
The investigation traced the gap through two causes — a timeout misclassified as a failure, a correlation that broke without the expected identifier, and a retry mechanism that couldn't resume after external success — each correct in isolation, each fixed independently.
...
**What changed my understanding.** The external system *had* processed it. The contract existed. The timeout told the application it hadn't received a response in time. It did not tell the application the operation had failed. Those are different facts, and treating them as the same thing is what started the chain.
